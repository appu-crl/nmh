...
local setExternalAutoPlayLaggerState
local startTracers, stopTracers
local startPlatform, stopPlatform
local startDesync, stopDesync
...
local function disableSpeedBoostForAutoPlay()
    if Enabled.SpeedBoost then
        autoPlaySpeedBoostRestore = true
        Enabled.SpeedBoost = false
        if VisualSetters.SpeedBoost then VisualSetters.SpeedBoost(false, true) end
        if floatButtonReferences.SpeedBoost then floatButtonReferences.SpeedBoost(false) end
        stopSpeedBoost()
...
local function restoreSpeedBoostAfterAutoPlay()
    if autoPlaySpeedBoostRestore and not Enabled.AutoLeft and not Enabled.AutoRight then
        autoPlaySpeedBoostRestore = false
        Enabled.SpeedBoost = true
        if VisualSetters.SpeedBoost then VisualSetters.SpeedBoost(true, true) end
        if floatButtonReferences.SpeedBoost then floatButtonReferences.SpeedBoost(true) end
        startSpeedBoost()
    end
end
...
local function teleportDownNow(dropDistance)
    ...
    task.spawn(function()
        pcall(function()
            ...
            hrp.CFrame = hrp.CFrame * CFrame.new(0, -(dropDistance or 20), 0)
...
function startBatAimbot()
    ...
    Connections.batAimbot = RunService.Heartbeat:Connect(function()
        ...
        if target and torso then
            ...
            align.CFrame = CFrame.lookAt(h.Position, predictedPos)
            local dirToTarget = predictedPos - h.Position
            local dist = dirToTarget.Magnitude
            local meleeOffset = 5
            local standPos = dist > 0 and (predictedPos - (dirToTarget.Unit * meleeOffset)) or predictedPos
            local moveDir = standPos - h.Position
            local distToStand = moveDir.Magnitude
            local spd = getEclipseSpeed(Values.BatAimbotSpeed, 10, 140)
            if distToStand > 1 then
                h.AssemblyLinearVelocity = moveDir.Unit * spd
            else
                h.AssemblyLinearVelocity = targetVel
            end
            if distToStand <= 5 then
                if bat and bat.Parent == c and (now - lastBatSwing) >= BAT_SWING_COOLDOWN then
                    lastBatSwing = now
                    pcall(function() bat:Activate() end)
                end
            end
...
local function setRaknetDesyncState(state)
    local env = (getgenv and getgenv()) or _G
    local rk = rawget(_G, "raknet") or rawget(env, "raknet")
    if type(rk) == "table" and typeof(rk.desync) == "function" then
...
function startDesync()
    Enabled.Desync = true
    setRaknetDesyncState(true)
end

function stopDesync()
    Enabled.Desync = false
    setRaknetDesyncState(false)
end
...
local stealPath_Right = {
    {pos = Vector3.new(-476.16, -6.52, 25.62)},
    {pos = Vector3.new(-483.06, -5.03, 27.51)},
    {pos = Vector3.new(-476.21, -6.63, 27.46)},
    {pos = Vector3.new(-476.66, -6.39, 92.44)},
    {pos = Vector3.new(-481.94, -5.03, 92.42)}
}
local stealPath_Left = {
    {pos = Vector3.new(-476.48, -6.28, 92.73)},
    {pos = Vector3.new(-482.85, -5.03, 93.13)},
    {pos = Vector3.new(-475.68, -6.89, 92.76)},
    {pos = Vector3.new(-476.50, -6.46, 27.58)},
    {pos = Vector3.new(-482.42, -5.03, 27.84)}
}
...
local function finishCashAutoPlay(side)
    if side == "left" then
        Enabled.AutoLeft = false
        if VisualSetters.AutoLeft then VisualSetters.AutoLeft(false, true) end
        if floatButtonReferences.AutoLeft then floatButtonReferences.AutoLeft(false) end
    ...
    setExternalAutoPlayLaggerState(false, false)
    updateStealPathProgress("READY", 0)
    restoreSpeedBoostAfterAutoPlay()
end

local function stopCashAutoPlay(side)
    ...
    if not Enabled.AutoLeft and not Enabled.AutoRight then
        setExternalAutoPlayLaggerState(false, false)
        restoreSpeedBoostAfterAutoPlay()
    end
end

local function startCashAutoPlay(side)
    stopCashAutoPlay(side)
    local points = side == "left" and stealPath_Left or stealPath_Right
    if side == "left" then autoLeftPhase = 1 else autoRightPhase = 1 end

    local conn
    conn = RunService.Heartbeat:Connect(function()
        if not stealPathActive or ((side == "left" and not Enabled.AutoLeft) or (side == "right" and not Enabled.AutoRight)) then
            return
        end
        local hrp = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
        local hum = Player.Character and Player.Character:FindFirstChildOfClass("Humanoid")
        ...
        local phase = side == "left" and autoLeftPhase or autoRightPhase
        local pointData = points[phase]
        local tgt = pointData and pointData.pos
        if not tgt then
            stopCashAutoPlay(side)
            finishCashAutoPlay(side)
            return
        end

        local flat = Vector3.new(tgt.X - hrp.Position.X, 0, tgt.Z - hrp.Position.Z)
        if flat.Magnitude < 1 then
            if phase == 5 then
                hum:Move(Vector3.zero, false)
                hrp.AssemblyLinearVelocity = Vector3.zero
                stopCashAutoPlay(side)
                local faceTarget = side == "left" and FACE_FAP_L or FACE_FAP_R
                if faceTarget then
                    local lookDir = Vector3.new(faceTarget.X, hrp.Position.Y, faceTarget.Z) - hrp.Position
            ...
            elseif phase == 2 then
                ...
                task.delay(0.05, function()
                    if side == "left" and Enabled.AutoLeft and stealPathActive then
                        autoLeftPhase = 3
                    elseif side == "right" and Enabled.AutoRight and stealPathActive then
                        autoRightPhase = 3
                    end
                end)
                return
                ...
                if side == "left" then
                    autoLeftPhase = phase + 1
                else
                    autoRightPhase = phase + 1
                end
                return
            end
        end

        local spd = phase >= 3 and getEclipseSpeed(Values.StealPathReturnSpeed, 1, 90) or getEclipseSpeed(Values.StealPathSpeed, 1, 120)
        setExternalAutoPlayLaggerState(phase == 4, true)
        local md = flat.Unit
        hum:Move(md, false)
        hrp.AssemblyLinearVelocity = Vector3.new(md.X * spd, hrp.AssemblyLinearVelocity.Y, md.Z * spd)
        updateStealPathProgress(phase >= 3 and "RETURNING TO BASE" or "AUTO PLAY", math.clamp(phase / #points, 0.03, 1))
    end)
...
function startStealPath(path)
    local hrp = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
    disableSpeedBoostForAutoPlay()
    STEAL_PATH_VELOCITY_SPEED = Values.StealPathSpeed
    STEAL_PATH_SECOND_SPEED = Values.StealPathReturnSpeed
    stealPathRunId += 1
    stealPathActive = true
    setExternalAutoPlayLaggerState(false, true)
    stealPathStartPosition = hrp.Position
    updateStealPathProgress("AUTO PLAY READY", 0.03)
    if path == stealPath_Left then
        stopCashAutoPlay("right")
        startCashAutoPlay("left")
    else
        stopCashAutoPlay("left")
        startCashAutoPlay("right")
    end
end
...
setExternalAutoPlayLaggerState = function(active, autoplayActive)
    autoPlayLongSegmentLaggerActive = active and true or false
    local env = getgenv and getgenv() or _G
    if type(env) == "table" then
        env.F7_AUTO_PLAY_ACTIVE = autoplayActive and true or false
        env.F7_AUTO_PLAY_LONG_LAGGER = autoPlayLongSegmentLaggerActive
    end

    if Enabled.LaggerCounter then
        if autoPlayLongSegmentLaggerActive then
            startLaggerCounter()
        else
            stopLaggerCounter()
        end
...
CreateToggle(ScrollFrame, "Lagger Counter Mode", "LaggerCounter", function(s)
    ...
    if s and (not stealPathActive or autoPlayLongSegmentLaggerActive) then
        startLaggerCounter()
    else
        stopLaggerCounter()
    end
...
CreateToggle(ScrollFrame, "No Tool Desync", "Desync", function(s)
    Enabled.Desync = s
...
local boxPanel = Create("Frame", {
    BackgroundTransparency = 1,
    Position = UDim2.new(1, -172, 0.5, -118),
    Size = UDim2.new(0, 150, 0, 236),
    BorderSizePixel = 0,
...
local function CreateBoxMobileButton(labelText, enabledKey, col, row, onToggle, isToggle)
    ...
    box.MouseButton1Click:Connect(function()
        if not Enabled.LockFloatPosition and moved then return end
        if isToggle then
...
floatButtonReferences.Desync = CreateFloatingButton("Desync", "⚙️", UDim2.new(1, -144, 0.5, -148), "Desync", function(state)
    Enabled.Desync = state
    if VisualSetters.Desync then VisualSetters.Desync(state, true) end
...
local function applySavedState()
    syncStealSettings()
    if Enabled.LaggerCounter then
        if not stealPathActive or autoPlayLongSegmentLaggerActive then
            startLaggerCounter()
        else
            stopLaggerCounter()
        end
...
Player.CharacterAdded:Connect(function()
    ...
    if Enabled.LaggerCounter and (not stealPathActive or autoPlayLongSegmentLaggerActive) then startLaggerCounter() end
